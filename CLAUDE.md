...
- `mpremote cp`（文件上传）在 USB Mode 下正常工作。
...
4. **`import rgb` 不存在** — 网上一些 M5Stack 示例使用 `import rgb` 控制 LED 灯条，但该模块在当前 UIFlow 固件中不存在。**正确方式：** 使用标准 `neopixel` 模块：
   ```python
   ...
   np.write()
   ```
5. **UIFlow 模块通过 `from m5stack import *` 加载** — 这会注入 `lcd`、`speaker`、`btnA`/`btnB`/`btnC` 等全局对象，不需要单独 import。不要尝试 `import lcd` 会报错。
